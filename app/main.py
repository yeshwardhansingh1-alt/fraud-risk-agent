...
try:
    ...
    policy_engine = ActionPolicyEngine(opt_threshold=0.32)
...
async def startup_event():
    # Attempt to connect, but don't crash if Redis isn't running (so tests can run)
    try:
...
async def evaluate_transaction(payload: dict, background_tasks: BackgroundTasks):
    ...
    # 3. Model Inference & SHAP Explainability
    if scorer:
        prob, reasons = scorer.predict_and_explain(feature_df)
        decision = policy_engine.evaluate(prob, payload.get("amount", 0.0), reasons)
    else:
        prob, reasons = 0.0, []
    ...
    # 4. Async Non-Blocking Write to SQLite Audit Ledger
